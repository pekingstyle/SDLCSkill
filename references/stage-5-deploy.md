...
3. **评审与实现分离（多实例角色隔离）**：REVIEW.md 由**全新上下文的评审实例**出具——推荐跑在 CI 非交互实例里（作者连评审运行环境都碰不到，workflow 文件受分支保护），最轻也可用 reviewer 子代理。评审实例的输入**只有** diff + plan.md + spec.md + 政策，不见作者对话、不采信作者的自我评价；作者实例只负责逐条修复、push，直到 CI 绿且 findings 清零。
...
## 本阶段禁令
自批（含作者实例出具或修改评审结论）；越过生产闸门；评审写表扬或超字数；省略 diff-vs-plan 检查。
